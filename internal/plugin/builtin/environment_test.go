...
func TestEnvironmentPluginInsertsBasicMessageBeforeUserMessage(t *testing.T) {
	environmentPlugin := NewEnvironmentPlugin(EnvironmentConfig{
	...
	if !strings.Contains(got.Messages[0].Content, "Weather: 晴, 30.0C, Humidity 80%, Wind 10.0 km/h") {
		t.Fatalf("environment message does not include weather: %q", got.Messages[0].Content)
	}
	if strings.Contains(got.Messages[0].Content, "农历:") || strings.Contains(got.Messages[0].Content, "[奇门遁甲 snapshot begin]") {
		t.Fatalf("environment message contains metaphysics data: %q", got.Messages[0].Content)
	}
...
func TestEnvironmentPluginWeatherFailureDoesNotBlockInsertion(t *testing.T) {
	environmentPlugin := NewEnvironmentPlugin(EnvironmentConfig{Location: "深圳", Timezone: "Asia/Shanghai", Weather: environmentWeatherStub{err: errors.New("down")}})
